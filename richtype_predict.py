...
originaltestX = testX[prediction_index]
...
#            n_epoch=100, batch_size=32)
model.load('model_richtypes')


def vec2label(labels, vec):
    max_index = vec.index(max(vec))
    return labels[max_index]

print('original test input:', originaltestX[0])
print('test input:', testX[prediction_index])
output = model.predict(testX[prediction_index].reshape([-1,100]))
print('test output:', output[0])
print('original test output:', vec2label(original_labels, output[0]))
